...
const List: React.FC = () => {
  ...
  const onDelete = (record: APIStayCategories.Data) => {
    if (data) {
      ...
      Loop.byId(temp, record.id, (item: APIStayCategories.Data) => item.loading_deleted = true);
      setData(temp);
    }
    ...
    doDelete(record.id)
      ...
      .finally(() => {
        if (data) {
          const temp: APIStayCategories.Data[] = [...data];
          Loop.byId(temp, record.id, (item: APIStayCategories.Data) => item.loading_deleted = false);
          setData(temp);
        }
      });
  };
  ...
  const onEnable = (record: APIStayCategories.Data) => {
    if (data) {
      const temp: APIStayCategories.Data[] = [...data];
      Loop.byId(temp, record.id, (item: APIStayCategories.Data) => item.loading_enable = true);
      setData(temp);
    }
    ...
    doEnable(enable)
      .then((response: APIResponse.Response<any>) => {
        if (response.code !== Constants.Success) {
          ...
          notification.success({ message: `类型${enable.is_enable === 1 ? '启用' : '禁用'}成功！` });
          if (data) {
            const temp = [...data];
            Loop.byId(temp, record.id, (item: APIStayCategories.Data) => item.is_enable = enable.is_enable);
            setData(temp);
          }
        }
      })
      .finally(() => {
        if (data) {
          const temp = [...data];
          Loop.byId(temp, record.id, (item: APIStayCategories.Data) => item.loading_enable = false);
          setData(temp);
        }
      });
  };
  ...
  return (
    <>
      <Card title='类型列表' extra={<Row gutter={10}>
        <Col>
          <Tooltip title='刷新'>
        ...
        {
          initialState?.permissions && initialState?.permissions?.indexOf('dormitory.stay.category.create') >= 0 ?
            <Col>
              <Tooltip title='创建'>
        ...
        <Table dataSource={data} rowKey='id' loading={loadingPaginate} pagination={false}>
          ...
          <Table.Column title='入住' render={(record: APIStayCategories.Data) => (
            record.is_temp === 1 ? '临时入住' : '正式入住'
          )} />
          <Table.Column title='序号' render={(record: APIStayCategories.Data) => (
            <Tag color={initialState?.settings?.primaryColor}>{record.order}</Tag>
          )} />
          <Table.Column title='启用' align='center' render={(record: APIStayCategories.Data) => (
            <Switch size='small' checked={record.is_enable === 1} onClick={() => onEnable(record)}
                    disabled={initialState?.permissions && initialState?.permissions?.indexOf('dormitory.stay.category.enable') < 0}
                    loading={record.loading_enable} />
          )} />
          <Table.Column title='创建时间' render={(record: APIStayCategories.Data) => (
            record.created_at && moment(record.created_at).format('YYYY/MM/DD')
          )} />
          <Table.Column align='center' width={100} render={(record: APIStayCategories.Data) => (
            <>
              {
                initialState?.permissions && initialState?.permissions?.indexOf('dormitory.stay.category.update') >= 0 ?
                  <Button type='link' onClick={() => onUpdate(record)}>编辑</Button> : <></>
              }
              {
                initialState?.permissions && initialState?.permissions?.indexOf('dormitory.stay.category.delete') >= 0 ?
                  <Popconfirm
                    title='确定要删除该数据?'
                    placement='leftTop'
