...
const List: React.FC = () => {
    ...
    doEnable(enable)
      .then((response: APIResponse.Response<any>) => {
        if (response.code !== Constants.Success) {
          ...
          notification.success({ message: `房型${enable.is_enable === 1 ? '启用' : '禁用'}成功！` });
          if (data) {
            const temp = [...data];
            Loop.byId(temp, record.id, (item: APIBasicTypes.Data) => item.is_enable = enable.is_enable);
            setData(temp);
          }
        }
      })
  ...
  return (
    <>
      <Card title='房型列表' extra={<Row gutter={10}>
        <Col>
          <Tooltip title='刷新'>
        ...
        {
          initialState?.permissions && initialState?.permissions?.indexOf('dormitory.basic.type.create') >= 0 ?
            <Col>
              <Tooltip title='创建'>
        ...
        <Table dataSource={data} rowKey='id' loading={loadingPaginate} pagination={false}>
          ...
          <Table.Column title='序号' render={(record: APIBasicTypes.Data) => (
            <Tag color={initialState?.settings?.primaryColor}>{record.order}</Tag>
          )} />
          <Table.Column title='启用' align='center' render={(record: APIBasicTypes.Data) => (
            <Switch size='small' checked={record.is_enable === 1} onClick={() => onEnable(record)}
                    disabled={initialState?.permissions && initialState?.permissions?.indexOf('dormitory.basic.type.enable') < 0}
                    loading={record.loading_enable} />
          )} />
          <Table.Column title='创建时间' render={(record: APIBasicTypes.Data) => (
            moment.isMoment(record.created_at) && record.created_at.format('YYYY/MM/DD')
          )} />
          <Table.Column align='center' width={100} render={(record: APIBasicTypes.Data) => (
            <>
              {
                initialState?.permissions && initialState?.permissions?.indexOf('dormitory.basic.type.update') >= 0 ?
                  <Button type='link' onClick={() => onUpdate(record)}>编辑</Button> : <></>
              }
              {
                initialState?.permissions && initialState?.permissions?.indexOf('dormitory.basic.type.delete') >= 0 ?
                  <Popconfirm
                    title='确定要删除该数据?'
                    placement='leftTop'
